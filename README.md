...
4. Are my files names correctly? Is there something that differentiates QC files from the samples?


5. Have I read this readme until the end?
...
path_data_in <- "T:/shares/di04_limet_bioinformatics/PhD Pablo/Scripts/TaPEx/Tutorial_example/Example_files" 
#here you should enter the path to your spreadsheet (.xlsx) with targeted compounds
masslist <- read_excel("T:/shares/di04_limet_bioinformatics/PhD Pablo/Publicaties/WIP/Gut_Phenotyping/Database/Database_fecal_metabolomics.xlsx")
...
Here you can also review the format of the database.

```{r}
knitr::kable(head(masslist))

```

You can add as many info in the database as you like, but for this script the folowing columns need to be present, with the exact names:
...
For the most common usecase, I would suggest this: make sure the QC's have someting that differentiate them from the samples, but also have a common element so they will also be included in the final intensity table.
E.g.
...
QC_list <- list(c(1,2),c(3,4))# you can add more days!
