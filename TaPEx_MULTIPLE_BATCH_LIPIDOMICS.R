...
path_data_in <- "P:/shares/di04_limet_flexigut/FGFP/Polar_method met raw/raw-polar/mzML_neg" #ADJUST THE PATH TO YOUR DATA PATH
setwd(path_data_in)
...
masslist_negative <- masslist[grep("M-",masslist$`Ion adduct`,fixed = T),]
...
# write.xlsx(masslist_negative, file = "mabvsslistneg.xlsx")
...
#First run this part to split lipidomic files in low and high group
...
#WHEN THIS PART IS FINISHED REMOVE NON CLEANED FILES FROM THE DIRECTORY AND CONTINUE AS YOU WOULD FOR POLAR 


#Seperate samples from QC's & STDS 
#MAKE SURE YOUR  FILES ARE NAMED ACCORDINGLY
msRawData_samples <-  datafiles[grep(pattern = "ss",datafiles)]
...
#EXTENSION: running several batches in a row:
QC_list <- list(c(1,3,5),c(6,10,11),c(12,15,17),c(18,19,21),c(24,27,29))#Indicate the QC files for each batch: must equal the amount of batchs you define later!
batch_list <- list(c(1:52),c(53:119),c(120:179),c(180:239),c(240:301)) #Indicate sample files per batch
